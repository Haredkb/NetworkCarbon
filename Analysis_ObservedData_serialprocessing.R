...
if(var == "Y"){
            ...
            #set up time steps
            timesteps = 364 #days, set up for non-leap year
...
scenarios_temperature <- lapply(scen, function(scen_temp){
  temp_sin <- scen_T[scen_temp][[1]]
    ...
    ##############################
    timesteps = 2* (24 * 365) -1 #minus one hour to end on correct day, first value is number of year to run for, code will always toss at least one year. 
    ts_units = "hour" #hour'
    # Set up model run times start dates
    ...
    ### Read in Standing Stock CPOM 
    # # This is supersecded as model standing stock is no longer initialized, but keep for reference and may be needed to speed up model runs. 
    ...
    #these loops are if running different initial days - this is generally superceded but keep for options to change. 
    # for all specified start days run the model 
    for(j in seq(1,length(intial_dates))){
              ...
              net_lst <- lapply(dates, function(t_s, env = parent.frame(), inherits = FALSE){
                              ...
                              ##ts_pre determines what was the previous time step. "ifelse" preserves type and class of inputs, and this catches daylight savings
          ...
          ### INITIAL INPUT
                          ...
                          ################
                            ...
                            ### Add Litter POC (direct and lateral) in
                             ...
                             ### Add Terrestrial Input DOC
                                ...
                                #Set POC in - using both direct and lateral is essential, as direct is only m2 which is strongly dependant on the width estimate, while lateral is consistent as assocaited wiht length
                                ...
                                #"A" for Acer and "M" for Microbes "S" for Shredders
                                ...
                                #write over Microbe dataframe
                                mod_df$Type = "Shredders"
                                mod_df$rhodo_acer = "A"
                                mod_AF <- predict(mod_k,newdata=mod_df,re.form=~0)#treat as population, so no RE
                                
                                #output into the network dataframe 
                                V(network)$k_AM <- exp(mod_AM)#transform from ln; micrboes
                                V(network)$k_AF <- exp(mod_AF); #shredders/ fragment
                                V(network)$k_At <- V(network)$k_AM + V(network)$k_AF #total 
                                ...
                                #message("No PRE or New day") #for debugging
                              ...
                              if(!exists("network_pre")){
                                ...
                                #Fragmented 
                                V(network)$POC_loss_AFDMg_F <-  V(network)$POC_sStock_AFDMg * (V(network)$k_AF/24)#hourly
                                ...
                                #Microbial
                                V(network)$POC_loss_AFDMg_M <-  V(network)$POC_sStock_AFDMg * (V(network)$k_AM/24)#hourly
                              ...
                              #Remaining Standing Stock for time step. This is thestanding stock used for next timestep
                    ...
                    ##set up environment for next timestep
                      ...
                      })# End TimeStep as part of NetLst
              ...
              #remove first list in network list as there is no transport
              # net_lst[[1]] <- NULL
              ...
              #create dataframe will all nodes for all timesteps. 
  ...
  pivot_longer(col = 2:4) #%>%#4)%>%
    ...
    return(scen_temp) #just return name to know it was run - take too much ram to keep data
}
)#end scenario lapply
