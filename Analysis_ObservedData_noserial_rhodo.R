...
scenarios_temperature <- lapply(scen, function(scen_temp){
  temp_sin <- scen_T[scen_temp][[1]]
    ...
    saveRDS(ts_all, paste0("output/data/rhodo/network_ts_all_", scen_temp, "_noserialC.RDS"))
    ...
    ggsave(plot = p,filename =  paste0("output/figures/rhodo/CBalance_timeseries_", scen_temp, ".png"))
    ...
    saveRDS(network_ts_day_df, paste0("output/data/rhodo/network_ts_day_df_", scen_temp, "_noserialC.RDS"))
    ...
    ggsave(plot = p, filename = paste0("output/figures/rhodo/Cbalance_monthlyavg_", scen_temp, ".png"))
    
    
    return(scen_temp)#ts_all take too make ram on long runs
}
)#end scenario lapply
#saveRDS(scenarios_temperature, "output/data/rhodo/base_serialC.RDS")
rm(list=setdiff(ls(), "scenarios_temperature"))
saveRDS(scenarios_temperature, "output/data/rhodo/scenarios_temperature__noserialC.RDS")
